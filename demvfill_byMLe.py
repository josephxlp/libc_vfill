...
def mask_invalid_values(data, nodata_value=-9999, min_valid=-30, max_valid=10000):
    """Masks out invalid values by setting them to NaN."""
    data = np.where((data <= min_valid) | (data >= max_valid) | (data == nodata_value), np.nan, data)
    return data

def interpolate_missing_values(data, model_type='catboost', n_seeds=5):
...
if __name__ == '__main__':
    ...
    for tilename in tilenames:
        ...
        dem_opath = f"{tile_odpath}/{tilename}_tdem_DEM__iMLe.tif"
